    ...
    <AppShell>
      ...
      <div className="space-y-4 p-5">
        {roteiros.map((r) => (
          <div
            key={r.id}
            className="relative overflow-hidden rounded-3xl bg-card shadow-soft transition hover:shadow-elevated"
          >
            <div className="relative aspect-[16/9] overflow-hidden">
              ...
              <div className="absolute inset-x-0 bottom-0 p-5 text-white">
                <div className="flex items-center gap-2 text-xs opacity-90">
                  <span className="flex items-center gap-1 rounded-full bg-white/20 px-2 py-0.5 backdrop-blur">
                    <Clock className="h-3 w-3" /> {r.duration}
                  </span>
                  <span className="flex items-center gap-1 rounded-full bg-accent px-2 py-0.5">
                    <Sparkles className="h-3 w-3" /> {r.vibe}
                  </span>
                </div>
